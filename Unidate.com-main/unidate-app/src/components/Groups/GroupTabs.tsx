...
const GroupTabs: React.FC<GroupTabsProps> = ({ groupId, isMember, isEditor }) => {
  ...
  const loadContent = useCallback(async () => {
    if (!isMember) return;
    try {
      const [nextMaterials, nextResources, nextAnnouncements] = await Promise.all([
        GroupMaterialsService.getGroupMaterials(groupId), GroupResourcesService.getGroupResources(groupId), GroupAnnouncementsService.getGroupAnnouncements(groupId),
      ]);
      setMaterials(nextMaterials); setResources(nextResources); setAnnouncements(nextAnnouncements);
  ...
  useEffect(() => {
    ...
    void Promise.all([GroupChatService.getChatStats(groupId), GroupEventsService.getGroupEvents(groupId, currentUser?.uid)])
      .then(([chat, events]) => setStats({ posts: 0, messages: chat.totalMessages, events: events.length }))
  ...
  return <div className="mt-6">
    ...
    {activeTab === 'calendar' && <section className="rounded-lg border border-gray-200 bg-white p-6"><h3 className="mb-2 text-lg font-semibold">Calendário do grupo</h3><p className="text-gray-600">Os eventos são exibidos na seção de eventos acima. Criadores e editores podem criar eventos ali.</p></section>}
