...
const Settings: React.FC = () => {
  const { userProfile, currentUser, logoutUser, refreshUserProfile } = useAuth();
  ...
  const handlePhotoUpload = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      reader.onload = (e) => {
        ...
        setProfileData(prev => ({ ...prev, photoURL: preview }));
      };
      reader.readAsDataURL(file);
      setSelectedFile(file);

      showSuccess('Foto selecionada! Clique em Salvar para atualizar.');
    } catch (error: any) {
    ...
    try {
      ...
      if (selectedFile) {
        try {
          ...
          showError('Erro ao salvar foto. Continuando sem atualizar a foto...');
        }
      }
            ...
            <div className="w-20 h-20 bg-gradient-to-br from-primary-500 to-accent-500 rounded-2xl flex items-center justify-center overflow-hidden">
              {photoPreview || userProfile?.photoURL ? (
                <img 
                  ...
                  onError={(e) => {
                    console.error('Erro ao carregar foto:', photoPreview || userProfile?.photoURL);
                    e.currentTarget.style.display = 'none';
                  }}
                />
              ) : (
                <span className="text-white text-2xl font-bold">
                  {userProfile?.displayName?.charAt(0)?.toUpperCase() || currentUser?.displayName?.charAt(0)?.toUpperCase() || currentUser?.email?.charAt(0)?.toUpperCase() || 'U'}
                </span>
              )}
            </div>
            <label className="absolute -bottom-2 -right-2 p-2 bg-white rounded-full shadow-lg border-2 border-gray-200 hover:bg-gray-50 transition-colors duration-200 cursor-pointer">
              {uploadingPhoto ? (
    ...
    <div className="space-y-6">
      ...
      <div>
        ...
        <div className="grid grid-cols-3 gap-4">
          <button className="relative p-4 border-2 border-primary-500 rounded-lg bg-white">
          ...
          <div className="relative p-4 border-2 border-gray-200 rounded-lg bg-gray-50 opacity-60 cursor-not-allowed">
            ...
            <span className="text-sm font-medium text-gray-500">Automático</span>
          </div>
        </div>
      </div>
    </div>
